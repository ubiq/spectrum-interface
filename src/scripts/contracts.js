...
var events = {
  ...
  '0x8be0079c': 'OwnershipTransferred(index_topic_1 address previousOwner, index_topic_2 address newOwner)',
  '0x8c5be1e5': 'Approval(index_topic_1 address owner, index_topic_2 address spender, uint256 value)',
  '0xddf252ad': 'Transfer(index_topic_1 address from, index_topic_2 address to, uint256 value)',
  // multisig (Fusion)
  ...
  '0x92ca3a80': 'SingleTransact(address owner, uint256 value, address to, bytes data)',
  '0xe7c957c0': 'MultiTransact(address owner, bytes32 operation, uint256 value, address to, bytes data)',
  '0x1733cbb5': 'ConfirmationNeeded(bytes32 operation, address initiator, uint256 value, address to, bytes data)',
  // escher
  '0x5ff467d6': 'onClaim(index_topic_1 address claimer)',
  '0x3fa7ed1e': 'onVote(index_topic_1 address voter, index_topic_2 uint256 candidate)',
  // trex controller
...
module.exports = {
  processTxnInput: function (str) {
    var methodId = str.substr(0, 10).toLowerCase()
  ...
  processEventLogs: function (logs_) {
    let logs = []
    logs_.forEach(function (item, index, arr) {
      var eventId = item.topics[0].substr(0, 10).toLowerCase()
      var isKnown = false
      var name = ''
      if (events[eventId]) {
        isKnown = true
